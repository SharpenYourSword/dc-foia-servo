...
def download_file(url, cookiedict, fname, ftype):	
	...
	#    subprocess.check_call(["curl", "-XPOST","-H 'content-length:0'", "-o", local_filename, new_url])
	#    subprocess.call("curl -XPOST -H 'Transfer-Encoding: chunked' -o " + dl + " " + new_url, shell=True)
	    try:
...
while (still_more):
	...
	out += objs
	for obj in objs:
		print "printing " + obj['fname']
		f = download_file("https://foia-dc.gov/Request/palContentType.aspx?type=3&DocID=" + obj["fname"] + "&isPALDoc=F", cookies, obj["fname"], obj["ftype"])
	try:
